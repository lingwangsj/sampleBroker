...
currentAccount: any;
    ...
    loadAll() {
        this.wlanGroupService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<WlanGroupMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/wlan-group-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/wlan-group-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
